...
class QERead(Read):
    ...
    def parse_info(self, store: bool = True, storeFolder: str = "./wfc/"):
        """
        ...
        for index, ks_node in enumerate(ks_nodes):
            ...
            for ispin in range(nspin):
                eigenvalues[ispin, index, :] = eigenvalue_[
                    int(np.sum(nbnd[:ispin])) : int(np.sum(nbnd[: ispin + 1]))
                ]
            ...
            for ispin in range(nspin):
                occupations[ispin, index, :] = occupation_[
                    int(np.sum(nbnd[:ispin])) : int(np.sum(nbnd[: ispin + 1]))
                ]
    ...
    def parse_wfc(self, real_space=False, storeFolder="./wfc/"):
        """
        analyze QE wfc*.dat
            param:
                file_name: str
        ...
        saveFolder = self.qe_outputFolder
        wfc_files = [
            saveFolder + "/" + file for file in os.listdir(saveFolder) if "wfc" in file
        ]
            ...
            if not hdf5:
                ...
                with open(file_name, "rb") as f:
                    ...
                    if rank == 0:
                        ...
                        for i in range(nbnd):
                            evc[i, :] = np.fromfile(
                                f, dtype="complex128", count=npol * igwx
                            )
                            f.seek(8, 1)
                    # step out of files
                if rank == 0:
                ...
                if self.comm:
                    recvbuf = np.empty(
                        (len(ibnd_loc[rank]), npol * igwx), dtype="complex128"
                    )
                    self.comm.Scatterv(
                        [send_data, count, displ, MPI.COMPLEX16], recvbuf, root=0
                    )
                else:
                    recvbuf = evc
                ...
                if rank == 0:
                    pbar = tqdm(
                        desc=f"store wfc of spin:{ispin:^3}/{self.xml_data['nspin']:^3}; ik:{ik:^3}/{self.xml_data['nks']:^3}",
                        total=nbnd,
                    )
                for index, i in enumerate(ibnd_loc[rank]):
                    if real_space:
                        ...
                        if gamma_only:
                            ...
                            evc_g[-mill[1:, 0], -mill[1:, 1], -mill[1:, 2]] = np.conj(
                                recvbuf[index, 1:]
                            )
                ...
                if self.comm:
                    recvbuf = np.empty(
                        (len(ibnd_loc[rank]), wfc_dict["npol"] * wfc_dict["igwx"]),
                        dtype="complex128",
                    )
                    self.comm.Scatterv(
                        [send_data, count, displ, MPI.COMPLEX16], recvbuf, root=0
                ...
                if rank == 0:
                    pbar = tqdm(
                        desc=f"store wfc of ispin:{wfc_dict['ispin']:^3}/{self.xml_data['nspin']:^3}; ik:{wfc_dict['ik']:^3}/{self.xml_data['nks']:^3}",
                        total=wfc_dict["nbnd"],
                    )
                for index, i in enumerate(ibnd_loc[rank]):
                    if real_space:
    ...
    def read(
        ...
        self.parse_info(store=True, storeFolder=storeFolder)
        self.parse_wfc(storeFolder=storeFolder, real_space=real_space)
...
def perturb_struct(
    ...
    assert os.path.exists(
        initial_pwfname
    ), f"initial_pwfname {initial_pwfname} does not exist"
    if out_fname is None:
        out_fname = os.path.join(
            pathlib.Path(initial_pwfname).parent,
            "perturb_" + pathlib.Path(initial_pwfname).name,
        )
        ...
        while idx < len(lines):
            ...
            if "ATOMIC_POSITIONS" in line:
                ...
                for i, atom in enumerate(atoms.get_chemical_symbols()):
                    f.write(
                        f"{atom:^10} {atom_positions[i, 0]:^14.8f} {atom_positions[i, 1]:^14.8f} {atom_positions[i, 2]:^14.8f}\n"
                    )
                    idx += 1
            else:
                f.write(line)
...
if __name__ == "__main__":
    ...
    qe = QERead(
        comm=comm,
        outFolder="/project/gagalli/jiaweiz/test/TEST_WEST/tutorials/h-bn/vacuum_extrapolate/vacuum_20/hbn.save/",
    )
    qe.parse_info()
