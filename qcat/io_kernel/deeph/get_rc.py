"""
Taken from DeepH-Pack
https://github.com/mzjb/DeepH-pack/blob/main/deeph/preprocess/get_rc.py
"""
...
def _get_local_coordinate(
    eij, neighbours_i, gen_rc_idx=False, atom_j=None, atom_j_R=None, r2_rand=False
):
    if gen_rc_idx:
        ...
        assert (
            atom_j_R is not None
        ), "atom_j_R must be specified when gen_rc_idx is True"
    else:
        rc_idx = None
    if r2_rand:
        r2_list = []
    ...
    r2_flag = None
    for r2, r2_index, r2_R in zip(
        neighbours_i.eijs[1:], neighbours_i.indices[1:], neighbours_i.Rs[1:]
    ):
        if torch.norm(torch.cross(r1, r2, dim=-1)) > 1e-6:
            ...
            if r2_rand:
                if (len(r2_list) == 0) or (
                    torch.norm(r2_list[0]) + 0.5 > torch.norm(r2)
                ):
                    r2_list.append(r2)
                else:
                    break
    ...
    assert (
        r2_flag is not None
    ), "There is no linear independent chemical bond in the Rcut range, this may be caused by a too small Rcut or the structure is 1D"
    if r2_rand:
    ...
    local_coordinate_2 = torch.cross(r1, r2, dim=-1) / torch.norm(
        torch.cross(r1, r2, dim=-1)
    )
    local_coordinate_3 = torch.cross(local_coordinate_1, local_coordinate_2, dim=-1)
    return torch.stack(
        [local_coordinate_1, local_coordinate_2, local_coordinate_3], dim=-1
    ), rc_idx
...
def get_rc(
    ...
    if_require_grad=False,
    cart_coords=None,
):
    if not if_require_grad:
        assert os.path.exists(
            os.path.join(input_dir, "site_positions.dat")
        ), "No site_positions.dat found in {}".format(input_dir)
        cart_coords = torch.tensor(
            np.loadtxt(os.path.join(input_dir, "site_positions.dat")).T
        )
    else:
        assert (
            cart_coords is not None
        ), 'cart_coords must be provided if "if_require_grad" is True'
    assert os.path.exists(
        os.path.join(input_dir, "lat.dat")
    ), "No lat.dat found in {}".format(input_dir)
    lattice = torch.tensor(
        np.loadtxt(os.path.join(input_dir, "lat.dat")).T, dtype=cart_coords.dtype
    )
    ...
    if gen_rc_by_idx != "":
        ...
        assert os.path.exists(
            os.path.join(gen_rc_by_idx, "rc_idx.h5")
        ), "Atomic indices for constructing rc rc_idx.h5 is not found in {}".format(
            gen_rc_by_idx
        )
        ...
        for key_str, rc_idx in fid_rc_idx.items():
            ...
            r1 = (
                cart_coords[rc_idx[0]]
                + torch.tensor(rc_idx[1:4]).type(cart_coords.dtype) @ lattice
                - cart_coords_i
            )
            r2 = (
                cart_coords[rc_idx[4]]
                + torch.tensor(rc_idx[5:8]).type(cart_coords.dtype) @ lattice
                - cart_coords_i
            )
            local_coordinate_1 = r1 / torch.norm(r1)
            local_coordinate_2 = torch.cross(r1, r2, dim=-1) / torch.norm(
            ...
            local_coordinate_3 = torch.cross(
                local_coordinate_1, local_coordinate_2, dim=-1
            )

            rc_dict[key_str] = torch.stack(
                [local_coordinate_1, local_coordinate_2, local_coordinate_3], dim=-1
            )
        fid_rc_idx.close()
    else:
        ...
        if create_from_DFT:
            assert os.path.exists(
                os.path.join(input_dir, neighbour_file)
            ), "No {} found in {}".format(neighbour_file, input_dir)
            ...
            for key_str in fid_OLP.keys():
                ...
                cart_coords_i = cart_coords[atom_i]
                cart_coords_j = (
                    cart_coords[atom_j] + R.type(cart_coords.dtype) @ lattice
                )
            ...
            for atom_i, neighbours_i in neighbours_dict.items():
                ...
                neighbours_i.dists = torch.tensor(
                    neighbours_i.dists, dtype=cart_coords.dtype
                )
                ...
                assert np.allclose(
                    neighbours_i.eijs[0].detach(),
                    torch.zeros_like(neighbours_i.eijs[0]),
                ), "eijs[0] should be zero"

                for R, eij, atom_j, atom_j_R in zip(
                    neighbours_i.Rs,
                    neighbours_i.eijs,
                    neighbours_i.indices,
                    neighbours_i.Rs,
                ):
                    key_str = str(list([*R.tolist(), atom_i + 1, atom_j.item() + 1]))
                    if gen_rc_idx:
                        rc_dict[key_str], rc_idx_dict[key_str] = _get_local_coordinate(
                            eij, neighbours_i, gen_rc_idx, atom_j, atom_j_R
                        )
                    else:
                        rc_dict[key_str] = _get_local_coordinate(
                            eij, neighbours_i, r2_rand=r2_rand
                        )[0]
        else:
