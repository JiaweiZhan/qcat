...
def parse_matrix(matrix_path, element, site_norbits, orbital_types_dict, factor=1.0):
    ...
    with open(matrix_path, "r") as f:
        ...
        for line in f:
            ...
            if num_element != 0:
                ...
                hamiltonian_cur = csr_matrix(
                    (
                        np.array(line2).astype(float),
                        np.array(line3).astype(int),
                        np.array(line4).astype(int),
                    ),
                    shape=(norbits, norbits),
                ).toarray()
                    ...
                    for index_site_j in range(nsites):
                        ...
                        mat = hamiltonian_cur[
                            (
                                ...
                                - site_norbits[index_site_i]
                            ) : site_norbits_cumsum[index_site_i],
                            (
                                site_norbits_cumsum[index_site_j]
                                - site_norbits[index_site_j]
                            ) : site_norbits_cumsum[index_site_j],
                        ]
                        mat *= factor
                        # if abs(mat).max() < 1e-8:
                        #     continue
                        mat = U_orbital.transform(
                            mat,
                            orbital_types_dict[element[index_site_i]],
...
def restore_matrix(
    hamiltonian_path, element, site_norbits, orbital_types_dict, factor=1.0
):
    ...
    hamiltonian = np.zeros(
        (site_norbits_cumsum[-1], site_norbits_cumsum[-1]), dtype=float
    )
    for index_site_i in range(nsites):
        for index_site_j in range(nsites):
            ...
            hamiltonian[
                (
                    site_norbits_cumsum[index_site_i] - site_norbits[index_site_i]
                ) : site_norbits_cumsum[index_site_i],
                (
